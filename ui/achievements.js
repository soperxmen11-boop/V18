...
let session = { totalCorrect:0, streak:0, colorCorrect:0, mascotCorrect:0, playedColor:false, playedMascot:false,
  ...
  function armRoundTimer(){
    if (session.roundTimerId) return;
    session.inRound = true;
    session.roundStartAt = Date.now();
    session.roundEndAt = session.roundStartAt + ROUND_MS;
    session.roundTimerId = setTimeout(()=>{
      session.roundEnded = true;
      const sc = (window.KPProgress && KPProgress.getState)? KPProgress.getState().score : (session.roundScore||0);
      const lvl = (window.KPProgress && KPProgress.getState)? KPProgress.getState().level : 1;
      const data = { correct: session.totalCorrect, wrong: session.roundWrong, score: sc, level: lvl };
      const outcome = (sc >= 10) ? 'pass' : 'fail';
      if (window.KPStats && typeof KPStats.open==='function'){ try{ KPStats.open({...data, outcome}); }catch(_){} }
      else { try{ openStats(); }catch(_){} }
  ...
  function bindDetectors(){
    document.addEventListener('click', (e)=>{ const el = e.target.closest('[data-correct]');
      ...
      if (el.getAttribute('data-correct') === '1'){
        ...
        session.streak = 0; session.roundWrong++;
      }
      checkAll();
      checkEndRound();
    }, true);
  ...
  function ensureStats(){
    let modal = document.getElementById('kp-stats');
    if (modal) return modal;
    modal = document.createElement('div'); modal.id='kp-stats'; modal.className='kp-modal';
    modal.innerHTML = `<div class="kp-card kp-stats" style="position:relative;">
      <button class="kp-close" aria-label="Close">✖</button>
      <h2>📊 Round Stats</h2>
      <div class="row">
        <div class="pill">✅ Correct: <span id="stCorrect">0</span></div>
        <div class="pill">❌ Wrong: <span id="stWrong">0</span></div>
        <div class="pill">🏆 Score: <span id="stScore">0</span></div>
        <div class="pill">📶 Level: <span id="stLevel">1</span></div>
      </div>
      <div class="row"><button class="kp-btn secondary" id="stAchv">🏅 Achievements</button></div>
      <div class="kp-actions">
        <button class="kp-btn primary" id="stRetry">🔄 Retry</button>
        <button class="kp-btn" id="stMenu">🏠 Menu</button>
      </div>
    </div>`;
    document.body.appendChild(modal);
    modal.addEventListener('click', (e)=>{ if (e.target===modal || e.target.classList.contains('kp-close')) modal.classList.remove('open'); });
  ...
  function openStats(){
    const modal = ensureStats();
    modal.querySelector('#stCorrect').textContent = session.totalCorrect;
    modal.querySelector('#stWrong').textContent = session.roundWrong;
    modal.querySelector('#stScore').textContent = (window.KPProgress?.getState()?.score || 0);
    modal.querySelector('#stLevel').textContent = (window.KPProgress?.getState()?.level || 1);
    modal.classList.add('open');
  }
